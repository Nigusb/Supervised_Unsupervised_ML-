...
# Calculate Gini Coefficient
GiniKNN <- Gini(y_pred = diabetesDataKnnPrediction, y_true = diabetesDataTesting$Outcome)
...
# Calculate Gini Coefficient
GiniNB <- Gini(y_pred = diabetesDataNbPrediction, y_true = diabetesDataTesting$Outcome)
...
# Calculate Gini Coefficient
GiniDT <- Gini(y_pred = diabetesDecisonTreePrediction, y_true = diabetesDataTesting$Outcome)
...
MDSplot(rf_model, diabetesDataTraining$Outcome, 
        pch = as.numeric(diabetesDataTraining$Outcome), palette = rep(1,3))
...
rf_modelBest <- randomForest(
  ...
  data = diabetesDataTraining, 
  ntree = 500,       
  mtry = 4,          # best parameter from the tuning 
  importance = TRUE,
  proximity = TRUE 
)
print(rf_modelBest)
...
# Calculate Gini Coefficient
GiniRF <- Gini(y_pred = predictions_rf_retrained, y_true = diabetesDataTesting$Outcome)
...
# Calculate Gini Coefficient
GiniSVM <- Gini(y_pred = predictions_svm_retrained, y_true = diabetesDataTesting$Outcome)
...
# Calculate Gini Coefficient
GiniXGB <- Gini(y_pred = binary_predictions_XGB_best, y_true = diabetesDataTesting$Outcome)
...
# Calculate Gini Coefficient
GiniNN <- Gini(y_pred = diabetesDataNeuralNetPrediction, y_true = diabetesDataTesting$Outcome)
